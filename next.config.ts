...
const nextConfig: NextConfig = {
  output: 'standalone',
  // Use webpack for next-pwa compatibility (Turbopack doesn't support webpack plugins yet)
  webpack: (config) => {
    // next-pwa will add its webpack configuration
    return config;
  },
  // Empty turbopack config to silence warning (we're using webpack for PWA)
  turbopack: {},
  images: {
    // Disable Next.js image optimization so Firebase Storage URLs are loaded directly
    // This avoids 400 errors from the _next/image optimizer in production.
    unoptimized: true,
  },
  // Enable compression
  compress: true,
...
const pwaConfig = withPWA({
  ...
  runtimeCaching: [
    ...
    {
      urlPattern: /^https:\/\/.*\.firebasestorage\.(?:googleapis|app)\.com\/.*/i,
      handler: 'CacheFirst',
      options: {
