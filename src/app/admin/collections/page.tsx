...
const CollectionList = () => {
  ...
  const fetchCollections = async () => {
    try {
      ...
      setError(t('admin.collections_fetch_failed') || 'فشل جلب المجموعات.');
      ...
      <React.Fragment key={collection.id}>
        <tr className="hover:bg-gray-50">
          <td className="px-4 sm:px-6 py-4 whitespace-nowrap">
            <input
              type="checkbox"
              checked={collection.id ? selectedIds.has(collection.id) : false}
              onChange={(e) => collection.id && handleSelectItem(collection.id, e.target.checked)}
          ...
          <td className="px-4 sm:px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900" style={{ paddingLeft: `${24 + indent}px` }}>
            <div className="flex items-center gap-2">
              ...
              {collection.children && collection.children.length > 0 && (
                <span className="text-xs text-gray-400 ml-2">({t('admin.collections_subcollections_count', { count: collection.children.length.toString() }) || `${collection.children.length} sub-collections`})</span>
              )}
            </div>
          </td>
          ...
          <td className="px-4 sm:px-6 py-4 whitespace-nowrap text-sm text-gray-500">
            {collection.parentCollection ? (
              <span className="text-xs text-gray-400">{t('admin.collections_subcollection') || 'المجموعة الفرعية'}</span>
            ) : (
              <span className="text-xs text-green-600 font-medium">{t('admin.collections_top_level') || 'المستوى الأعلى'}</span>
            )}
          </td>
  ...
  const handleDelete = async (id: string) => {
    ...
    setConfirmDialogMessage(t('admin.collections.delete_confirm') || 'هل أنت متأكد أنك تريد حذف هذه المجموعة؟');
    setConfirmDialogAction(async () => {
      try {
        ...
        setInfoDialogMessage(t('admin.collections_delete_success') || 'تم حذف المجموعة بنجاح!');
        setInfoDialogType('success');
        setShowInfoDialog(true);
      } catch {
        setInfoDialogMessage(t('admin.collections_delete_failed') || 'فشل حذف المجموعة.');
        setInfoDialogType('error');
        setShowInfoDialog(true);
      }
    });
  ...
  const handleBatchDelete = async () => {
    ...
    setConfirmDialogMessage(t('admin.collections_batch_delete_confirm', { count: count.toString() }) || `Are you sure you want to delete ${count} collection(s)?`);
    setConfirmDialogAction(async () => {
      try {
        const deletePromises = Array.from(selectedIds).map(id => deleteCollection(id));
        await Promise.all(deletePromises);
        setCollections(collections.filter(collection => !selectedIds.has(collection.id!)));
        setSelectedIds(new Set());
        fetchCollections();
        setInfoDialogMessage(t('admin.collections_batch_delete_success', { count: count.toString() }) || `${count} collection(s) deleted successfully!`);
        setInfoDialogType('success');
        setShowInfoDialog(true);
      } catch {
        setInfoDialogMessage(t('admin.collections_batch_delete_failed') || 'فشل حذف المجموعات.');
        setInfoDialogType('error');
        setShowInfoDialog(true);
      }
    });
  ...
  const handleSelectAll = (checked: boolean) => {
    if (checked) {
      const allIds = paginatedCollections.map(collection => collection.id!).filter(Boolean) as string[];
  ...
  const isAllSelected = paginatedCollections.length > 0 && paginatedCollections.every(collection => collection.id && selectedIds.has(collection.id));
  const isSomeSelected = paginatedCollections.some(collection => collection.id && selectedIds.has(collection.id));
  ...
  return (
    <div className="space-y-4 md:space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl sm:text-3xl font-semibold text-gray-900">{t('admin.collections') || 'المجموعات'}</h1>
          <p className="text-gray-500 text-sm mt-1">{t('admin.collections_subtitle') || 'إدارة مجموعات المنتجات والمجموعات الفرعية'}</p>
        </div>
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
          <div className="flex items-center gap-2 bg-gray-100 rounded-lg p-1">
            ...
            >
              {t('admin.collections_tree_view') || 'شجرة'}
            </button>
            <button
            ...
            >
              {t('admin.collections_flat_view') || 'مستوي'}
            </button>
          </div>
          <Link
          ...
          >
            ...
            {t('admin.collections_add_new') || 'أضف المجموعة'}
          </Link>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        {collections.length === 0 ? (
          <div className="p-8 sm:p-12 text-center text-gray-500">
            <p className="text-base sm:text-lg font-medium mb-2">{t('admin.collections_empty_title') || 'لم يتم العثور على مجموعات.'}</p>
            <p className="text-sm text-gray-400">{t('admin.collections_empty_message') || 'ابدأ بإنشاء مجموعتك الأولى'}</p>
          </div>
        ) : (
          <>
            {/* Desktop Table View */}
              ...
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-50">
                  <tr>
                    ...
                    <th scope="col" className="px-4 sm:px-6 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">{t('admin.collections_table_name') || 'الاسم'}</th>
                    <th scope="col" className="px-4 sm:px-6 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">{t('admin.collections_table_slug') || 'سبيكة'}</th>
                    <th scope="col" className="px-4 sm:px-6 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">{t('admin.collections_table_description') || 'الوصف'}</th>
                    <th scope="col" className="px-4 sm:px-6 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">{t('admin.collections_table_type') || 'النوع'}</th>
                    <th scope="col" className="px-4 sm:px-6 py-3 text-right text-xs font-semibold text-gray-600 uppercase tracking-wider">{t('admin.collections_table_actions') || 'الإجراءات'}</th>
                  </tr>
                </thead>
                <tbody className="bg-white divide-y divide-gray-200">
                  {viewMode === 'tree' ? (
                    ...
                    paginatedCollections.map((collection) => {
                      ...
                      return (
                        <tr key={collection.id} className={`hover:bg-gray-50 transition-colors ${selectedIds.has(collection.id!) ? 'bg-blue-50' : ''}`}>
                          <td className="px-4 sm:px-6 py-4 whitespace-nowrap">
                            <input
                              type="checkbox"
                              checked={collection.id ? selectedIds.has(collection.id) : false}
                          ...
                          <td className="px-4 sm:px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                            {parentName ? (
                              <span className="text-xs text-gray-600">{t('admin.collections_sub_of') || 'فرعية من'}: {parentName}</span>
                            ) : (
                              <span className="text-xs text-green-600 font-medium">{t('admin.collections_top_level') || 'المستوى الأعلى'}</span>
                            )}
                          </td>
            ...
            <div className="md:hidden divide-y divide-gray-200">
              {viewMode === 'tree' ? (
                hierarchicalCollections.map(collection => {
                  const renderMobileCollection = (col: CollectionWithChildren, level: number = 0): React.ReactNode => (
                    <React.Fragment key={col.id}>
                      <div className="p-4 hover:bg-gray-50 transition-colors" style={{ paddingLeft: `${16 + level * 16}px` }}>
                        ...
                        <div className="flex items-center justify-center gap-2 pt-3 border-t border-gray-100 mt-3">
                          <button
                            onClick={() => router.push(`/admin/collections/edit/${col.id}`)}
                      ...
                      {col.children && col.children.map(child => renderMobileCollection(child, level + 1))}
                    </React.Fragment>
                  );
                  return renderMobileCollection(collection, 0);
                })
              ) : (
                paginatedCollections.map((collection) => {
                  const parentName = collections.find(c => c.id === collection.parentCollection)?.name;
                  return (
                    <div key={collection.id} className={`p-4 hover:bg-gray-50 transition-colors ${selectedIds.has(collection.id!) ? 'bg-blue-50' : ''}`}>
                      <div className="flex items-start gap-3 mb-2">
                        ...
                        />
                        <div className="flex-1 min-w-0">
                          <h3 className="text-sm font-semibold text-gray-900 mb-1">{collection.name}</h3>
                          ...
                          <div className="mt-2">
                            {parentName ? (
                              <span className="text-xs text-gray-600">{t('admin.collections_sub_of') || 'فرعية من'}: <span className="font-medium">{parentName}</span></span>
                            ) : (
                              <span className="text-xs text-green-600 font-medium">{t('admin.collections_top_level') || 'المستوى الأعلى'}</span>
                            )}
                          </div>
            ...
            {totalPages > 1 && (
              <div className="bg-gray-50 px-4 sm:px-6 py-3 border-t border-gray-200 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                <div className="text-sm text-gray-700">
                  {t('admin.common.showing') || 'عرض'} {startIndex + 1} {t('admin.common.to') || 'to'} {Math.min(endIndex, collections.length)} {t('admin.common.of') || 'من'} {collections.length} {t('admin.common.results') || 'نتائج'}
                </div>
                <div className="flex items-center gap-2">
                  <button
