...
const StatesPage = () => {
  ...
  const fetchStates = React.useCallback(async () => {
    ...
    const enrichedData = data.map(state => {
       ...
       return { ...state, countryName: country?.name || state.countryName || 'غير معروف' };
    });
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="text-center max-w-md px-4">
          <h1 className="text-3xl font-heading font-bold mb-4 text-gray-900">{t('admin.geography_states_not_available') || 'الولايات غير متاحة'}</h1>
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      resetForm();
      fetchStates();
      setInfoDialogMessage(editingId ? (t('admin.geography_states_update_success') || 'تم تحديث الولاية بنجاح!') : (t('admin.geography_states_create_success') || 'تم إنشاء الولاية بنجاح!'));
      setInfoDialogType('success');
      setShowInfoDialog(true);
    } catch {
      // Error saving state
      setInfoDialogMessage(t('admin.geography_states_save_failed') || 'فشل حفظ الولاية');
      setInfoDialogType('error');
      setShowInfoDialog(true);
    }
  };
  ...
  const handleDelete = async (id: string) => {
    ...
    setConfirmDialogMessage(t('admin.geography_states_delete_confirm') || 'هل أنت متأكد أنك تريد حذف هذه الولاية؟');
    setConfirmDialogAction(async () => {
      try {
        await deleteState(id);
        fetchStates();
        setSelectedIds(new Set());
        setInfoDialogMessage(t('admin.geography_states_delete_success') || 'تم حذف الولاية بنجاح!');
        setInfoDialogType('success');
        setShowInfoDialog(true);
      } catch {
        setInfoDialogMessage(t('admin.geography_states_delete_failed') || 'فشل حذف الولاية');
        setInfoDialogType('error');
        setShowInfoDialog(true);
      }
    });
  ...
  const handleBatchDelete = async () => {
    ...
    setConfirmDialogMessage(t('admin.geography_states_batch_delete_confirm', { count: count.toString() }) || `Are you sure you want to delete ${count} state(s)?`);
    setConfirmDialogAction(async () => {
      try {
        const deletePromises = Array.from(selectedIds).map(id => deleteState(id));
        await Promise.all(deletePromises);
        fetchStates();
        setSelectedIds(new Set());
        setInfoDialogMessage(t('admin.geography_states_batch_delete_success', { count: count.toString() }) || `${count} state(s) deleted successfully!`);
        setInfoDialogType('success');
        setShowInfoDialog(true);
      } catch {
        setInfoDialogMessage(t('admin.geography_states_batch_delete_failed') || 'فشل في حذف الدول.');
        setInfoDialogType('error');
        setShowInfoDialog(true);
      }
    });
  ...
  const handleSelectAll = (checked: boolean) => {
    if (checked) {
      const allIds = paginatedStates.map(state => state.id!).filter(Boolean) as string[];
  ...
  const isAllSelected = paginatedStates.length > 0 && paginatedStates.every(state => state.id && selectedIds.has(state.id));
  const isSomeSelected = paginatedStates.some(state => state.id && selectedIds.has(state.id));
  ...
  return (
    <div className="space-y-4 md:space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl sm:text-3xl font-semibold text-gray-900">{t('admin.geography_states') || 'الولايات / المقاطعات'}</h1>
          <p className="text-gray-500 text-sm mt-1">{t('admin.geography_states_subtitle') || 'إدارة معلومات الولاية والمقاطعة'}</p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
          ...
          >
            <option value="">{t('admin.geography_all_countries') || 'جميع الدول'}</option>
            {countries.map(c => (
          ...
          >
            ...
            {t('admin.geography_states_add') || 'إضافة ولاية'}
          </button>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        {states.length === 0 ? (
          <div className="p-8 sm:p-12 text-center text-gray-500">
            <p className="text-base sm:text-lg font-medium mb-2">{t('admin.geography_states_empty') || 'لم يتم العثور على ولايات.'}</p>
            <p className="text-sm text-gray-400">{t('admin.geography_states_empty_message') || 'ابدأ بإضافة حالتك الأولى'}</p>
          </div>
        ) : (
          <>
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-gray-50 border-b border-gray-200">
                    ...
                    <th className="px-4 sm:px-6 py-3 text-xs font-semibold text-gray-600 uppercase tracking-wider">{t('admin.geography_table_code') || 'الرمز'}</th>
                ...
                <tbody className="divide-y divide-gray-200">
                  {paginatedStates.map((state) => (
                    <tr key={state.id} className={`hover:bg-gray-50 transition-colors ${selectedIds.has(state.id!) ? 'bg-blue-50' : ''}`}>
                      <td className="px-4 sm:px-6 py-4 whitespace-nowrap">
                        <input
                          type="checkbox"
                          checked={state.id ? selectedIds.has(state.id) : false}
                          onChange={(e) => state.id && handleSelectItem(state.id, e.target.checked)}
                          className="w-4 h-4 text-gray-900 border-gray-300 rounded focus:ring-gray-900"
                        />
                      </td>
                      <td className="px-4 sm:px-6 py-4 text-sm text-gray-900 font-medium">{state.name}</td>
                      <td className="px-4 sm:px-6 py-4 text-sm text-gray-600">{state.code}</td>
                      <td className="px-4 sm:px-6 py-4 text-sm text-gray-600">
                        {countries.find(c => c.id === state.countryId)?.name || state.countryName || (t('admin.geography_unknown') || 'غير معروف')}
                      </td>
                      <td className="px-4 sm:px-6 py-4 text-sm">
                        <span className={`inline-flex items-center px-2.5 py-1 rounded-md text-xs font-semibold ${
                          state.status === 'active' ? 'bg-green-50 text-green-700' : 'bg-red-50 text-red-700'
                        }`}>
                          {state.status}
                        </span>
                      </td>
            ...
            <div className="md:hidden divide-y divide-gray-200">
              {paginatedStates.map((state) => (
                <div key={state.id} className={`p-4 hover:bg-gray-50 transition-colors ${selectedIds.has(state.id!) ? 'bg-blue-50' : ''}`}>
                  <div className="flex items-start gap-3 mb-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <h3 className="text-sm font-semibold text-gray-900 mb-1">{state.name}</h3>
                      <div className="space-y-1 text-xs text-gray-600">
                        <p>{t('admin.geography_table_code') || 'الرمز'}: {state.code}</p>
                        <p>{t('admin.geography_table_country') || 'الدولة'}: {countries.find(c => c.id === state.countryId)?.name || state.countryName || (t('admin.geography_unknown') || 'غير معروف')}</p>
                      </div>
                    </div>
            ...
            {totalPages > 1 && (
              <div className="bg-gray-50 px-4 sm:px-6 py-3 border-t border-gray-200 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                <div className="text-sm text-gray-700">
                  {t('admin.common.showing') || 'عرض'} {startIndex + 1} {t('admin.common.to') || 'to'} {Math.min(endIndex, states.length)} {t('admin.common.of') || 'من'} {states.length} {t('admin.common.results') || 'نتائج'}
                </div>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md p-4 sm:p-6 max-h-[90vh] overflow-y-auto">
            <h2 className="text-xl font-bold mb-6">{editingId ? (t('admin.geography_states_edit') || 'تحرير الدولة') : (t('admin.geography_states_add_new') || 'إضافة دولة جديدة')}</h2>
            
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">{t('admin.geography_form_country') || 'الدولة'}</label>
                <select 
                  required
                ...
                >
                  <option value="">{t('admin.geography_form_select_country') || 'اختر البلد'}</option>
                  {countries.map(c => (
                    <option key={c.id} value={c.id}>{c.name}</option>
                  ))}
                </select>
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">{t('admin.geography_form_state_name') || 'اسم الولاية'}</label>
                <input 
                  type="text" 
                  required
                  value={formData.name}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">{t('admin.geography_form_state_code') || 'رمز الولاية'}</label>
                  <input 
                    type="text" 
                    required
                    value={formData.code}
              ...
              <div className="flex justify-end gap-3 mt-6 pt-4 border-t border-gray-100">
                ...
                >
                  {editingId ? (t('admin.geography_states_update') || 'تحديث الولاية') : (t('admin.geography_states_add') || 'إضافة ولاية')}
                </button>
              </div>
            </form>
          </div>
